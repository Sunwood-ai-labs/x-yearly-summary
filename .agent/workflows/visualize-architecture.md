...
1. **プロジェクトの解析**
   - `list_dir` や `view_file` ツールを使用して、プロジェクトの構造、主要なファイル（README.md, package.json, main scripts等）を確認してください。
   - アプリケーションの目的、主要なコンポーネント、データの流れ、依存関係を特定してください。
   - **重要**: 物理的なファイル構造ではなく、READMEやドキュメントから読み取れる**論理的な概念**（Mission Control, Factory, Brainなど）を優先してください。

2. **Draw.io XMLの生成**
   - 解析した情報を基に、Draw.ioで読み込み可能なXMLテキストを生成してください。
   - **デザイン要件 (Strict Requirements)**:
     - **言語**: **英語 (English)** で統一してください。
     - **スタイル**: **「和風（Wafu/Miyabi）」なカラーパレット**を使用し、洗練されたプロフェッショナルな図にしてください。
       ...
       - **グラデーション禁止 (Flat Design Only)**: `gradientColor=none` を指定し、フラットなデザインにすること。
     - **構成**:
...
3. **ファイルの保存**
   - 生成したXMLコンテンツを `docs/architecture.drawio` というファイル名で保存してください。
     - ※ `docs` ディレクトリが存在しない場合は、ユーザーに作成を促すか、`write_to_file` で親ディレクトリ作成が可能ならそのまま保存してください。
   - 保存完了後、ユーザーに通知してください。

4. **(Optional) 画像のエクスポートと埋め込み**
